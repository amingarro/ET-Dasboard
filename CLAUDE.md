...
**Probar notas con `driveSyncEnabled: true` sincroniza con la cuenta real de Google Drive.** `useNotes().saveNote()` (`src/lib/notes.ts`) dispara automáticamente `window.electronAPI.drive.sync()` 2s después de cada guardado de nota, sin distinguir test de uso real — cualquier nota creada o editada mientras se prueba contra un perfil real con Drive sync activado se sube a la carpeta real `"ET Dashboard - Notas"` de esa cuenta, imágenes incluidas. Se descubrió creando notas de prueba descartables con una instancia de dev manejada por CDP durante la construcción de la feature de imágenes+sync de notas: las notas y sus imágenes realmente terminaron en la carpeta real de Drive. Si necesitás probar el comportamiento de notas/Drive sync, o desactivá el Drive sync primero (Configuración) o borrá las notas de prueba después a través del propio IPC `notes:delete` de la app — borrar desde la app las marca como tombstone (`electron/syncState.ts`), así el próximo sync también elimina la copia en Drive y las imágenes huérfanas en vez de dejarlas ahí.
...
- `npm test` — Vitest, unit tests only so far (pure functions: `migrateLayout`, `birthdayUtils`, `noteUtils` — no React/DOM tests exist yet). `.github/workflows/ci.yml` runs lint + `npm run build` + `npm test` on push/PR — **not** a separate standalone `tsc --noEmit`: `npm run build` already type-checks both the renderer (inside `next build`) and the main process (`build:electron`), and a bare `tsc --noEmit` run before `next build`/`next dev` has ever generated its typed-routes declarations (gitignored `next-env.d.ts` + `out/dev/types/**`) fails on a fresh checkout with `Cannot find name 'LayoutProps'`. `release.yml` is the separate, pre-existing publish pipeline and doesn't run any of these checks itself.
